...
5. 在同一页面的 **Variables** 添加 `OPENAI_MODEL`；当前默认值为
   `gpt-5.6-luna`。如果之前配置为 `gpt-5.4-mini` 或 `gpt-5.6-terra`，请一并更新该变量。
...
- 如果任务在写入过程中中断，重新运行会从发布日志恢复，并且只有索引与课程文件一致时才
  会公开。
- 在 GitHub Actions 日志中查看 `lesson-generation-failure` 错误码和
  `daily-pipeline-summary` 汇总；日志刻意不输出新闻全文、模型原文回复或 API 密钥。
